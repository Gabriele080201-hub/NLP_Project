...
import {
  initialOrderLines,
  initialExtractionNotes,
  logisticsNote,
  rawExtractedText,
  initialValidationSheet
} from '../mockData';
import { motion, AnimatePresence } from 'motion/react';

interface ReviewPageProps {
  currentLang: Language;
  onConfirmOrder: (summaryData: {
    customerCode: string;
...
export default function ReviewPage({ currentLang, onConfirmOrder, metadata }: ReviewPageProps) {
  const t = translations[currentLang];

  // Load state
  const [lines, setLines] = useState<OrderLine[]>(initialOrderLines);
  ...
  return (
    <div id="review-container" className="pb-16 font-sans">
      ...
      <div className="max-w-[1400px] mx-auto px-6 pt-8 space-y-8">
        ...
        <div id="section-notes-accordion" className="bg-white border border-[#E0E0E0] rounded-[8px] overflow-hidden shadow-sm">
          ...
          >
            <span className="text-[14px] font-bold text-[#1C2B3A] tracking-tight uppercase flex items-center space-x-2">
              <span>{t.extractionNotesTitle} ({initialExtractionNotes.length})</span>
            </span>
              ...
              >
                <div className="p-6 border-t border-[#E0E0E0] space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Note Card 1 (Normal White Card) */}
                    <div className="bg-white border border-[#E0E0E0] rounded-[8px] p-5 shadow-sm space-y-3">
                      <div className="flex items-center">
                        <span className="bg-stone-100 text-stone-600 text-[9px] font-bold px-2.5 py-0.5 rounded uppercase tracking-wider">
                          {t.extractionNotesBadgeExtracted}
                        </span>
                      </div>
                      <p className="text-xs font-mono text-stone-400 italic">
                        "{initialExtractionNotes[0].original}"
                      </p>
                      <p className="text-[13px] font-bold text-[#1C2B3A]">
                        {currentLang === 'IT' ? initialExtractionNotes[0].extracted.it : initialExtractionNotes[0].extracted.de}
                      </p>
                    </div>

                    {/* Note Card 2 (Uncertainty Yellow Card) */}
                    <div className="bg-[#FFFDE7] border border-[#FFF59D] rounded-[8px] p-5 shadow-sm space-y-3">
                      <div className="flex items-center space-x-2">
                        <span className="bg-amber-100 text-amber-800 text-[9px] font-bold px-2.5 py-0.5 rounded uppercase tracking-wider">
                          {t.extractionNotesBadgeExtracted}
                        </span>
                      ...
                      <p className="text-xs font-mono text-[#EF6C00]/80 italic">
                        "{initialExtractionNotes[1].original}"
                      </p>
                      <p className="text-[13px] font-bold text-[#1C2B3A]">
                        {currentLang === 'IT' ? initialExtractionNotes[1].extracted.it : initialExtractionNotes[1].extracted.de}
                      </p>
                    </div>
                  ...
                  <div className="bg-[#F8F9FA] border border-[#E0E0E0] p-4 rounded-[6px] text-xs leading-relaxed text-[#1C2B3A]">
                    ...
                    <span className="font-semibold text-stone-700">
                      {currentLang === 'IT' ? logisticsNote.it : logisticsNote.de}
                    </span>
                  </div>
        ...
        {/* Confirm Order Button - Right align */}
        <div className="flex justify-end select-none pt-4">
          <button
